...
export const getAllOrders = async (request: Request, response: Response) => {
    try {
        ...
        const allOrders = await prisma.order.findMany({
            ...
            include: { OrderList: true } /** include detail of order (item that sold) */
        })
 ...
 export const createOrder = async (request: Request, response: Response) => {
    try {
        ...
        const newOrder = await prisma.order.create({
            data: { uuid, customer, table_number, total_price, payment_method, status, user : user.idUser }
        })
        ...
        for (let index = 0; index < orderlists.length; index++) {
            ...
            await prisma.orderList.create({
                data: {
                    uuid, idOrder : newOrder.idOrder, idMenu : Number(menuId), quantity: Number(quantity), note
                }
            })
        }
        return response.json({
 ...
 export const deleteOrder = async (request: Request, response: Response) => {
  ...
  let deleteOrder = await prisma.order.delete({ where: { idOrder: Number(id) } })
